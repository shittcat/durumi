...
function SetMarker(inum) {
  ...
  kakao.maps.event.addListener(marker, "click", function () {
    ...
    var tmpcont = overlay
      .getContent()
      .replace(/itemOverlays noMark/gi, "itemOverlays setMark");
    ...
    if (viewMode == 1) {
      ...
      tmpcont = tmpcont.replace(
        /place/,
        'place" style="width: 70vw; height: 40vh;"'
      );
    } else if (viewMode == -1) {
      //picture 일때
      tmpcont = tmpcont.replace(
        /place/,
        'place" style="transform: rotateY(0deg);"'
      );
      tmpcont = tmpcont.replace(
        /picture/,
        'picture" style="width: 70vw; height: 40vh; transform: rotateY(0deg);"'
      );
    }
